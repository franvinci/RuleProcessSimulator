...
def reformat_dot_str(input_str, dot_distributions=False):
    ...
    if dot_distributions:
        ...
        # Function to round the numbers and replace the match with rounded values in parentheses
        def round_and_replace(match):
            num1, num2 = match.groups()  # Get the two numbers
            rounded1 = round(float(num1))  # Round the first number
            rounded2 = round(float(num2))  # Round the second number
            return f'({rounded1}, {rounded2})'  # Return formatted in parentheses
